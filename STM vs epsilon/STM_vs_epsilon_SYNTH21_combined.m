clear; clc; close all
path = '../datasets/';
load([path,'STM_data_K_10_D_50_N_100k.mat'])
w = w';
nensemble = 10;
...
for n_itr = 1:nensemble
    ...
    for eps_id = 1:length(epsilon_all)
        ...
        VDP_AG(:, :, eps_id) = myPreProcessProbabilityVects(VDP);
        lam_finalDPAG(1:K, eps_id) = sort(lambdaDP / sum(lambdaDP), 'ascend');
        errCAPE(n_itr, eps_id) = norm(w - lam_finalDPAG(1:K, eps_id));
        err_vecCAPE(n_itr, eps_id) = myEigVecCheck(mu, VDP_AG(:,:, eps_id), 1);
        
        VDP_AG_conv(:, :, eps_id) = myPreProcessProbabilityVects(VDP_conv);
        ...
        err_vecCONV(n_itr, eps_id) = myEigVecCheck(mu, VDP_AG_conv(:,:, eps_id), 1);
        
        VDP_AG_local(:, :, eps_id) = myPreProcessProbabilityVects(VDP_local);
        ...
        err_vecLOCAL(n_itr, eps_id) = myEigVecCheck(mu, VDP_AG_local(:,:, eps_id), 1);

    end
end
save('STM_vs_epsilon_K_10_D_50_N_100k','V','lam_final','VDP_AG', 'lam_finalDPAG', ...
