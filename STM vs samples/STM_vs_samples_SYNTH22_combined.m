...
for n_itr = 1:nensemble
    ...
    for n_id = 1:length(N_all)
        ...
        load([path,'STM_data_K_10_D_50_N_',num2str(N/1000),'k']);
        ...
        VNDP(:, :, n_id) = myPreProcessProbabilityVects(V);
        lam_final(:, n_id) = sort(lambda / sum(lambda), 'ascend');
        err(n_itr, n_id) = norm(w - lam_final(:, n_id));
        err_vec(n_itr, n_id) = myEigVecCheck(mu, V, 1);
        
        
        VDP_AG(:, :, n_id) = myPreProcessProbabilityVects(VDP);
        lam_finalDPAG(:, n_id) = sort(lambdaDP / sum(lambdaDP), 'ascend');
        errCAPE(n_itr, n_id) = norm(w - lam_finalDPAG(K, n_id));
        err_vecCAPE(n_itr, n_id) = myEigVecCheck(mu, VDP_AG(:, :, n_id), 1);
        
%         
        VDP_AG_conv(:, :, n_id) = myPreProcessProbabilityVects(VDP_conv);
        lam_finalDP_conv(:, n_id) = sort(lambdaDP / sum(lambdaDP_conv), 'ascend');
        errCONV(n_itr, n_id) = norm(w - lam_finalDP_conv(:, n_id));
        err_vecCONV(n_itr, n_id) = myEigVecCheck(mu, VDP_AG_conv(:, :, n_id), 1);
        
        VDP_AG_local(:, :, n_id) = myPreProcessProbabilityVects(VDP_local);
        lam_finalDP_local(:, n_id) = sort(lambdaDP / sum(lambdaDP_local), 'ascend');
        errLOCAL(n_itr, n_id) = norm(w - lam_finalDP_local(:, n_id));
        err_vecLOCAL(n_itr, n_id) = myEigVecCheck(mu, VDP_AG_local(:, :, n_id), 1);
        
    end
end

save('STM_vs_samples_K_10_D_50_eps_5','V','lam_final','VDP_AG', 'lam_finalDPAG', ...
