function [lambda,V] = my_orth_tensor_decomp_ver1 (T, C, K, epsilon, delta)
...
T_tilde = my_multilinear_tensor(T,1,W);
...
for k = 1:K
    ...
    elseif nargin < 5
        while flag
            u_hat = my_dp_multilinear_tensor(T_hat,u,epsilon);
            nrm = norm(u_hat);
            u_new = u_hat/nrm;
            change = norm(u_new-u);
            itr = itr+1;
        ...
        while flag
            u_hat = my_dp_multilinear_tensor(T_hat,u,epsilon,delta);
            nrm = norm(u_hat);
            u_new = u_hat/nrm;
            change = norm(u_new-u);
            itr = itr+1;
